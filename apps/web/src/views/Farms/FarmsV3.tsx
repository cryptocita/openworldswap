...
import { useCakeUserInfo } from './utils/useCakeUserInfo'
...
const Farms: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const userCakeInfo = useCakeUserInfo(
  ...
  const mockfarms = [
    {
      ...
      tokenAmountTotal: userCakeInfo.tokenAmountTotal,
      quoteTokenAmountTotal: userCakeInfo.quoteTokenAmountTotal,
      ...
      tokenPriceVsQuote: userCakeInfo.tokenPriceVsQuote,
  ...
  let farmsV2 = mockfarms as DeserializedFarm[]
  let v2UserDataLoaded = true
  let v2PoolLength = 1
  let regularCakePerBlock = 1

  const {
    data: farmsV2X,
    userDataLoaded: v2UserDataLoadedX,
    poolLength: v2PoolLengthX,
    regularCakePerBlock: regularCakePerBlockX,
  } = useFarms()

  if (chainId !== ChainId.OEX_TESTNET) {
    farmsV2 = farmsV2X
    v2UserDataLoaded = v2UserDataLoadedX
    v2PoolLength = v2PoolLengthX
    regularCakePerBlock = regularCakePerBlockX
  }
  ...
  const [v2FarmOnly, setV2FarmOnly] = useState(true)
  ...
  const activeFarms = useMemo(
    () =>
      farmsLP.filter(
        (farm) => farm.multiplier !== '0X' || (farm.version === 2 && farm?.bCakeWrapperAddress),
  ...
  // console.log('sdfsdf', chosenFarms, activeFarms)
