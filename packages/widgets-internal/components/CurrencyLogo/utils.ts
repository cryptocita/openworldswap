...
export const getTokenListBaseURL = (chainId: number) =>
  `https://app.openworldswap.finance/images/${chainName[chainId]}`;

export const getTokenListTokenUrl = (token: Token) =>
  Object.keys(chainName).includes(String(token.chainId))
    ? `https://app.openworldswap.finance/images/${token.chainId === ChainId.BSC ? "" : `${chainName[token.chainId]}/`}${
        token.address
      }.png`
    : null;
...
export const getCommonCurrencyUrlBySymbol = memoize(
  (symbol?: string): string | undefined =>
    symbol && commonCurrencySymbols.includes(symbol)
      ? `https://app.openworldswap.finance/images/symbol/${symbol.toLocaleLowerCase()}.png`
      : undefined,
