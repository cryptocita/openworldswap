...
export const getOnBoardingDescriptionMessage = (isOnBoarded: boolean, t: TranslateFunction) => {
  let onBoardingDescription: string = t(
    'Finally, subscribe to notifications to stay informed on the latest news and updates that OpenWorldSwap has to offer.',
  )
  if (!isOnBoarded) {
    onBoardingDescription = t(
      'Get started with notifications from OpenWorldSwap. First authorize notifications by signing in your wallet',
    )
  }
...
export const getOnBoardingButtonText = (
  ...
  if (isStep3) return t('Subscribe To OpenWorldSwap')
